...
class ExcelHandler(object):
    ...
    def read_excel(self,sheet_name,sheet_path):
        '''
        读取固定excel,返回指定sheet内容
        :return:
        '''
        book = xlrd.open_workbook(sheet_path)        # 注意：这里已经写死了是接口测试对应的文件路径，并没有参数化
        table = book.sheet_by_name(sheet_name)       # 通过sheet名字获取数据
        return table

    def get_excel_data(self,sheet_name,module,sheet_path=Config.API_CASE_PATH):
        '''
        可以通过参数：file_name文件名来区分不同数据，对应不同的函数入口
         数据文件过滤excel中不必要的数据
        :param file_name: 通过他来区分是接口还是web 数据文件
        :param sheet_name: excel的sheet表名
        :param case_desc: 通过excel中的case_description来过滤用例
        :return:
        '''
        sheet = self.read_excel(sheet_name,sheet_path)
        rows, cols = sheet.nrows, sheet.ncols
        l = []
        title = sheet.row_values(0)
        # 获取其他行
        for i in range(1, rows):
            if module in sheet.row_values(i):                # 这里过滤指定用例，否则所有用例都会放到一个case中执行
