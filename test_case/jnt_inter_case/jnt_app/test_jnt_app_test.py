...
class TestJntAppTest(object):
    ...
    @pytest.mark.parametrize('case', excel.get_excel_data("jnt_app_test",'Login'))
    def test_app_test_Login(self,case):
        self.new.test_case_method(case)

    @allure.story('OA模块')
    @pytest.mark.jnt_app
    # @pytest.mark.parametrize()第一个参数为逗号分隔的字符串列表，第二个参数是值列表parametrize()
    @pytest.mark.parametrize('case', excel.get_excel_data("jnt_app_test",'OA'))
    def test_app_test_oa(self,case):
